      ...
      <main className="max-w-xl mx-auto px-4 pb-24">
        <form onSubmit={onSubmit} className="mt-6 space-y-5">
          ...
          <div className="relative">
            <select value={values.eventId} onChange={e => set('eventId', e.target.value)} onBlur={() => mark('eventId')} className="w-full rounded-xl bg-white/10 border border-white/15 px-4 py-3 outline-none focus:border-orange-400 appearance-none">
          ...
          <button disabled={!canSubmit} type="submit" className="w-full rounded-full bg-orange-500 disabled:bg-orange-500/50 text-white px-6 py-3 shadow-lg shadow-orange-500/30 hover:bg-orange-400 transition">
            Register
          </button>
        </form>
      </main>
...
function FloatingInput({ id, label, value, onChange, onBlur, type = 'text', error }: { id: string; label: string; value: string; onChange: (v: string) => void; onBlur: () => void; type?: string; error?: string }) {
  return (
    <div className="relative">
      <input id={id} name={id} value={value} onChange={(e) => onChange(e.target.value)} onBlur={onBlur} type={type} placeholder=" " className="w-full rounded-xl bg-white/10 border border-white/15 px-4 py-3 outline-none focus:border-orange-400 peer" />
      <label htmlFor={id} className="absolute left-3 top-1/2 -translate-y-1/2 px-1 text-white/70 pointer-events-none transition-all peer-focus:-top-2 peer-focus:text-xs peer-focus:text-orange-300 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-placeholder-shown:text-sm bg-[#081a33]">
        {label}
      </label>
...
function Success() {
  return (
    <div className="min-h-screen bg-[#081a33] text-white grid place-items-center px-4">
      <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', stiffness: 150, damping: 12 }} className="text-center">
        ...
        <a href="/events" className="inline-block mt-6 rounded-full bg-orange-500 text-white px-6 py-3 hover:bg-orange-400">Back to Events</a>
      </motion.div>
    </div>
  )
}
