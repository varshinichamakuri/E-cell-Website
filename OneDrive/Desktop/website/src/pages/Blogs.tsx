...
const tagOptions = ['All', 'Startup', 'Innovation', 'Events'] as const
    ...
    <div className="min-h-screen bg-[#081a33] text-white">
      ...
      <section className="relative overflow-hidden">
        <motion.div aria-hidden className="absolute inset-0" initial={{ opacity: 0 }} animate={{ opacity: 1, backgroundPosition: ['0% 50%', '100% 50%'] }} transition={{ duration: 14, repeat: Infinity, repeatType: 'reverse' }} style={{
          backgroundImage: 'radial-gradient(600px 300px at 10% 20%, rgba(251,146,60,.18), transparent 60%), radial-gradient(600px 300px at 90% 0%, rgba(59,130,246,.18), transparent 60%)',
        ...
        <div className="relative max-w-6xl mx-auto px-4 pt-28 pb-16 text-center">
          ...
          <p className="mt-3 text-white/85 max-w-2xl mx-auto">Ideas, recaps, and perspectives from the E-Cell community.</p>
        </div>
      </section>
      ...
      <main className="max-w-6xl mx-auto px-4 pb-24">
        ...
        <div className="flex items-center justify-center gap-2 rounded-full bg-white/10 border border-white/10 p-1 w-fit mx-auto">
          {tagOptions.map(option => (
            <button key={option} onClick={() => setTag(option)} className={`px-4 py-2 rounded-full text-sm transition ${tag === option ? 'bg-orange-500 text-white' : 'text-white/80 hover:text-white'}`}>
              {option}
            </button>
          ))}
        </div>
...
function BlogCard({ item }: { item: BlogItem }) {
  return (
    <motion.div whileHover={{ y: -6, scale: 1.01 }} className="rounded-2xl overflow-hidden border border-white/10 bg-white/5 backdrop-blur">
      ...
      <div className="p-5">
        <div className="text-xs text-white/70">By {item.author} • {item.date}</div>
        <h3 className="mt-1 text-lg font-semibold">{item.title}</h3>
        <p className="mt-2 text-white/80 text-sm">{item.preview}</p>
        <div className="mt-3 flex flex-wrap gap-2">
          {item.tags.map(t => (
            <span key={t} className="text-[11px] text-orange-300 bg-orange-500/20 px-2 py-1 rounded-full">{t}</span>
          ))}
        </div>
        <div className="mt-4">
          <Link to={`/blogs/${item.id}`} className="text-sm text-orange-300 hover:text-orange-200">Read More</Link>
        </div>
      </div>
    </motion.div>
  )
